...
def kbest(s, k): # using the division in total (no need to check duplicate), not in best
    ...
    def tryadd(split, index1, index2): # i paired with split-1
        if index1 < len(left(split)) and index2 < len(right(split)) \
           and not (split, index1, index2) in used:
            used.add((split, index1, index2))
            heappush(heap, 
                     (-left(split)[index1][0]-right(split)[index2][0]-1, split, index1, index2)) # pair

    def tryadd1(index):
        if index < len(single()):
            heappush(heap, (-single()[index][0], index)) # min-heap, i unpaired
        ...
        for i in range(n-span+1): # left boundary: 0..(n-span)
            ...
            while heap != [] and len(opt[i, j]) < k:
                ...
                if len(item) == 2: # i unpaired
                    value, index = item
                    opt[i,j].append((-value, ".%s" % single()[index][1])) 
                    tryadd1(index+1)
                else: # i paired, split
                    value, split, index1, index2 = item
                    opt[i,j].append((-value, "(%s)%s" % (left(split)[index1][1], 
                                                         right(split)[index2][1])))
...
if __name__ == "__main__":
    pp = lambda a: list(map(lambda n: n[0], a))    
    print(best("ACAGU"))
    print(total("ACAGU"))
    print(kbest("ACAGU", 10))
    print(pp(kbest("UCAGAGGCAUCAAACCU", 300)))
    print(pp(kbest("CAUCGGGGUCUGAGAUGGCCAUGAAGGGCAC", 25)))
    print(pp(kbest("AAACCAUCGGGGUCUGAGAUGGCCAUGAAGGGCA", 25))) 
    print(pp(kbest("UUUUUCCCCCAUCGGGGUCUGAGAUGGCCAUGA", 25)))
    print(pp(kbest("UUUUCCGGGGGGGGGCAUCGGGGUCUUUU", 25)))
    print(pp(kbest("UUUCUCUCUCUCCAUCGGGGUCUGAGAUGGCCAUGAA", 35)))
    print(pp(kbest("UCAGAGGCAUCAAACCUU", 700)))
